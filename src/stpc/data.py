...
# Note: Your utils should be organized. Let's assume create_noisy_clean_pair is in ecg_utils
from .utils.ecg_utils import create_noisy_clean_pair
...
class PhysioNetDataset(Dataset):
    ...
    This version is refactored to accept a dictionary of training parameters.
    """
    def __init__(self, clean_signals: list, noise_signals: dict, train_params: dict, num_samples_per_epoch: int):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Access parameters using dictionary keys - THIS IS THE FIX
        snr_db = random.uniform(
            self.train_params['snr_db_min'],
            self.train_params['snr_db_max']
        )
