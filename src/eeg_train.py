...
class BandMaskedFFTLoss(nn.Module):
    ...
    def forward(self, pred, target):
        # pred, target shape: [B, C, T]
        if self.mask is None:
            T = pred.shape[-1]
            freqs = torch.fft.rfftfreq(T, 1.0 / self.fs).to(pred.device)
            self.mask = ((freqs >= self.f_low) & (freqs <= self.f_high)).float()
            self.mask = self.mask[None, None, :] # for broadcasting to [B, C, F]
        
        Xp = torch.fft.rfft(pred, dim=-1)
        Xc = torch.fft.rfft(target, dim=-1)
        mag_p = torch.abs(Xp)
        mag_c = torch.abs(Xc)
    ...
    for noisy, clean in loop:
        ...
        with torch.autocast(device_type=config.DEVICE, dtype=torch.float16, enabled=config.DEVICE=='cuda'):
            ...
            if args.experiment == 'spatial':
                ...
                loss += (args.alpha * loss_temp) + (args.beta * loss_spat)
            
            # --- NEW: FREQUENCY EXPERIMENT LOGIC ---
            elif args.experiment == 'frequency':
                loss_alpha = loss_fns['Alpha'](denoised, clean)
                loss_low = loss_fns['Low'](denoised, clean)
                loss_high = loss_fns['High'](denoised, clean)
                # Weighted sum: Heavily penalize errors in Alpha band
                loss += (5.0 * loss_alpha) + (0.1 * loss_low) + (0.1 * loss_high)
            # ----------------------------------------
...
def main(args):
    ...
    training_segments = []
    # ... (data loading logic with held-out set remains the same)
    ...
    # --- NEW: LOSS FUNCTION DICTIONARY FOR FREQUENCY ---
