...
# --- NEW: Import the MaskedEEGDataset ---
from eeg_data_utils import load_eeg_from_edf, create_eeg_segments, EEGDataset, MaskedEEGDataset, TARGET_FS, get_adjacency_list

# ... (find_common_monopolar_channels and Loss classes remain the same) ...
def find_common_monopolar_channels(directory):
    common_channels_set = None; montage = mne.channels.make_standard_montage('standard_1020')
    standard_channels_map = {ch.upper(): ch for ch in montage.ch_names}; standard_channels_upper = set(standard_channels_map.keys())
    ...
    for f in tqdm(files_to_check, desc="Scanning for common channels"):
        ...
        try:
            ...
            for ch_name in raw.ch_names:
                ...
                if mono_name_upper in standard_channels_upper: current_monopolar_upper.add(mono_name_upper)
            if common_channels_set is None: common_channels_set = current_monopolar_upper
            else: common_channels_set.intersection_update(current_monopolar_upper)
...
class BandMaskedFFTLoss(nn.Module):
    def __init__(self, fs, f_low, f_high):
        super().__init__(); self.fs, self.f_low, self.f_high = fs, f_low, f_high
        self.loss = nn.L1Loss(); self.mask = None
    def forward(self, pred, target):
        if self.mask is None:
            T = pred.shape[-1]; freqs = torch.fft.rfftfreq(T, 1.0 / self.fs).to(pred.device)
            self.mask = ((freqs >= self.f_low) & (freqs <= self.f_high)).float()[None, None, :]
        mag_p = torch.abs(torch.fft.rfft(pred, dim=-1)); mag_c = torch.abs(torch.fft.rfft(target, dim=-1))
...
def train_one_epoch(loader, model, optimizer, loss_fns, scaler, config, args):
    ...
    total_loss = 0.0
    # --- NEW: Training loop for self-supervised is slightly different ---
    # The input is 'masked', the target is 'clean'
    for masked, clean in loop:
        masked, clean = masked.to(config.DEVICE), clean.to(config.DEVICE)
        
        with torch.autocast(device_type=config.DEVICE, dtype=torch.float16, enabled=config.DEVICE=='cuda'):
            # The model sees the masked input and tries to reconstruct the clean output
            denoised = model(masked)
            
            # The loss is always calculated against the original clean signal
            loss = loss_fns['L1'](denoised, clean) + (args.beta * loss_fns['Spatial'](denoised, clean))
...
def main(args):
    ...
    # --- NEW: For self-supervised, we train on EVERYTHING (seizure and non-seizure) ---
    all_segments = []
    for f in tqdm(os.listdir(subject_dir), desc="Loading and processing all files"):
        if f.endswith('.edf'):
    ...
    if not all_segments: raise RuntimeError("No segments created.")
    print(f"Total segments for self-supervised training: {len(all_segments)}")
    
    # --- NEW: Use the MaskedEEGDataset for this experiment ---
    if args.experiment == 'self_supervised':
        train_dataset = MaskedEEGDataset(clean_segments=all_segments, samples_per_epoch=config.SAMPLES_PER_EPOCH)
        print("Using MaskedEEGDataset for self-supervised learning.")
    else: # Fallback for other experiments (though we are focusing on self-supervised now)
    ...
    # --- NEW: Self-supervised loss combines reconstruction and spatial consistency ---
    adjacency_list = get_adjacency_list(common_channels)
    loss_functions = {'L1': nn.L1Loss(), 'Spatial': LaplacianLoss(adjacency_list)}
...
if __name__ == '__main__':
    ...
    # ... (parser arguments are the same, we just need to run with --experiment self_supervised)
    ...
    parser.add_argument('--alpha', type=float, default=1.0) # Not used in self_supervised
    parser.add_argument('--beta', type=float, default=1.0) # Used for spatial term
